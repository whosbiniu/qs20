...
const { targetQuarters, sessionTargetQuarters, excludedSessionWindow, fullWeekQuarter } = require('../public/cycle-rules.js');

// Q2/Q3 mapping remains unchanged outside the NY AM session exception.
...
assert.deepEqual(sessionTargetQuarters(1), [1, 4]);
assert.deepEqual(sessionTargetQuarters(2), [1, 3]);
assert.deepEqual(sessionTargetQuarters(3), [3]);
assert.deepEqual(sessionTargetQuarters(4), [1, 4]);
assert.equal(excludedSessionWindow(3, 1), true);
assert.equal(excludedSessionWindow(2, 1), false);
assert.equal(excludedSessionWindow(4, 1), false);
assert.equal(excludedSessionWindow(3, 3), false);
...
console.log('PASS NY AM excludes M90 Q1; London, NY PM and other cycle mappings remain unchanged');
